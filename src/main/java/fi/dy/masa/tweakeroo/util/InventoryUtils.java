...
public class InventoryUtils
{
    ...
    public static void trySwitchToWeapon(Entity entity)
    {
            ...
            TOOL_SWITCH_IGNORED_SLOTS.contains(player.inventory.selectedSlot) == false)
        {
            ...
            int slotNumber = findSlotWithBestItemMatch(container, test, new IntRange(36, 44), new IntRange(9, 35));
    ...
    private static float getBaseAttackDamage(ItemStack stack)
    {
        ...
        {
            return ((IMixinMiningToolItem) item).tweakeroo_getAttackDamage();
        }
    ...
    public static int getEnchantmentLevel(ItemStack stack, @Nonnull Enchantment enchantment)
    {
    ...
    private static boolean hasSameOrBetterWeaponEnchantments(ItemStack testedStack, ItemStack previousTool)
    {
        ...
        // Support
        count += hasSameOrBetterEnchantment(testedStack, previousTool, Enchantments.SWEEPING);
    ...
    private static int hasSameOrBetterEnchantment(ItemStack testedStack, ItemStack previous, Enchantment enchantment)
    {
    ...
    {
        return findSuitableSlot(container, itemTest, new IntRange(9, container.slots.size() - 1));
    }

    private static int findSuitableSlot(Container container, Predicate<ItemStack> itemTest, IntRange... ranges)
    {
    ...
    private static int findSlotWithBestItemMatch(Container container, ItemPickerTest itemTest, IntRange... ranges)
    {
        final int max = container.slots.size() - 1;
    ...
    private static int findEmptySlot(Container container, Collection<Integer> slotNumbers)
    {
    ...
    private static void swapItemToHand(PlayerEntity player, Hand hand, int slotNumber)
    {
        ...
        if (slotNumber != -1 && container == player.playerContainer)
        {
            MinecraftClient mc = MinecraftClient.getInstance();
    ...
    private static void swapToolToHand(int slotNumber, MinecraftClient mc)
    {
        ...
        if (slotNumber >= 0 && player.container == player.playerContainer)
        {
            PlayerInventory inventory = player.inventory;
            Container container = player.container;
    ...
    private static int getUsableHotbarSlotForTool(int currentHotbarSlot, Collection<Integer> validSlots, Container container)
    {
        int first = -1;
        int nonTool = -1;
    ...
    public static boolean canUnstackingItemNotFitInInventory(ItemStack stack, PlayerEntity player)
    {
            ...
            UNSTACKING_ITEMS.contains(stack.getItem()))
        {
            if (fi.dy.masa.malilib.util.InventoryUtils.findEmptySlotInPlayerInventory(player.playerContainer, false, false) == -1)
            {
                ...
                return fi.dy.masa.malilib.util.InventoryUtils.findEmptySlotInPlayerInventory(player.playerContainer, false, false) == -1;
            }
        }
    ...
    public static void switchToPickedBlock()
    {
        ...
        if (player == null || world == null || player.container != player.playerContainer)
        {
            return;
        }
